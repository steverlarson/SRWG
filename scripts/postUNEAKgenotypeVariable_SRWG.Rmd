...
```{r munge_one, eval=TRUE}

Input <- read.delim(file = '~/bigshare/Steve/Analysis_tools/1-TJ_SRWG/UNEAK/hapMap/HapMap.hmc.txt', header = TRUE, stringsAsFactors = FALSE, check.names = FALSE)
...
write.table(parsed.Output.Lim, file="~/bigshare/Steve/Analysis_tools/1-TJ_SRWG/UNEAK/hapMap/TJ_SRWG_2_6_30pctmiss.geno", sep="\t", row.names=TRUE, col.names=NA, quote=F)

TJ_SRWG_2_6_30pctmiss.geno <- read.table(file="~/bigshare/Steve/Analysis_tools/1-TJ_SRWG/UNEAK/hapMap/TJ_SRWG_2_6_30pctmiss.geno")
...
write.table(params_used, file="~/bigshare/Steve/Analysis_tools/1-TJ_SRWG/UNEAK/hapMap/postUNEAKgenotypeParams.txt", sep="\t", quote=F, row.names=F)
```
...
#library(polyRAD)
require(polyRAD)
mydata <- readHMC("~/bigshare/Steve/Analysis_tools/1-TJ_SRWG/UNEAK/hapMap/HapMap.hmc.txt", fastafile = "~/bigshare/Steve/Analysis_tools/1-TJ_SRWG/UNEAK/hapMap/HapMap.fas.txt")
GetTaxa(mydata)
...
# Reading Haplotypes distribution from:/media/bigdata/shared/Steve/Analysis_tools/1-TJ_SRWG/scripts/../UNEAK/tagCounts/Blank_HFM2LBGXN_0_DNA230105P02_B07_X5.cnt
# Number of Tags in file:4694
# Reading Haplotypes distribution from:/media/bigdata/shared/Steve/Analysis_tools/1-TJ_SRWG/scripts/../UNEAK/tagCounts/Blank_HFM2LBGXN_0_DNA230105P01_H01_X5.cnt
...
mydata_sub1$genotypeLikelihood[[1]][,1:12,1:6]
...
# remove DI-05 well G05, DI-08 well B06, and SR-07 is C06 look like off types. These offtypes were on plate DNA230105P01, which has contaminated blank
...
mypca$source <- as.factor(mypca$source) #make factor
plot(mypca$PC1, mypca$PC2, col = mypca$source, cex = 1,  pch = 18, xlab = 'PCA 1 (%)', ylab = 'PCA 2 (%)')
legend('bottomright', legend = unique(mypca$source), col = 1:length(mypca$source), pch = 18, cex = 1)
...
mydata_sub1$alleleFreq
mydata_sub1$alleles2loc
...
mydata_geno$genotypes[1:141,1:1000]
...
Export_Structure(mydata_sub1, '~/bigshare/Steve/Analysis_tools/1-TJ_SRWG/UNEAK/hapMap/poprad_structure_out.txt' , missingIfZeroReads = FALSE) #use missingIfZeroReads = FALSE to use prior probs for missing data

structure <- read.delim(file = '~/bigshare/Steve/Analysis_tools/1-TJ_SRWG/UNEAK/hapMap/poprad_structure_out.txt', header = TRUE, stringsAsFactors = FALSE, check.names = FALSE)
...
write.table(structure, file="~/bigshare/Steve/Analysis_tools/1-TJ_SRWG/UNEAK/hapMap/poprad_structure_out_pops.txt", sep="\t", row.names=FALSE, quote=F) #this can be converted to ARLEQUIN.arp file by PGDSpider
...
#use ARLEQUIN (C:\Users\Steve.Larson\Documents\WinArl35\WinArl35)

mydata_wtgeno <- GetWeightedMeanGenotypes(mydata_sub1)
mydata_wtgeno[1:100,1:100]
Export_TASSEL_Numeric(mydata_sub1, '~/bigshare/Steve/Analysis_tools/1-TJ_SRWG/UNEAK/hapMap/poprad_tassel_numeric_out.txt' ,)
